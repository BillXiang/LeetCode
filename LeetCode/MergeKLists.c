...
//void minHeadAdd(int* heap, int heapSize, struct ListNode** lists, int listsIndex){
//
//}
//
//int heapTop(int* heap){
//
//    return -1;
//}
//
//struct ListNode* mergeKListsWithHeap(struct ListNode** lists, int listsSize) {
//    struct ListNode result = {INT_MIN, NULL};
//    struct ListNode* tail = &result;
//    int* minHeap[listsSize];
//    int heapSize = 0;
//    int i = 0;
//    for(i=0; i<listsSize; i++){
//        if(lists[i]!=NULL){
//            minHeadAdd(minHeap, heapSize, lists, i);
//            heapSize++;
//        }else{
//            if(i<listsSize-1){
//                lists[i] = lists[i+1];
...
//        if(lists[min]!=NULL){
//            minHeadAdd(minHeap, heapSize, lists, min);
//            heapSize++;
//        }else{
//            if(min<listsSize-1){
...
struct ListNode* binaryMerge(struct ListNode* list1, struct ListNode* list2, int isASC){
    if(list1==NULL){
        return list2;
    }
    if(list2==NULL){
...
struct ListNode* mergeKListsDC(struct ListNode** lists, int start, int end) {
    int mid = 0;
    if(start==end){
        return lists[start];
    }else if(start+1==end){
        return binaryMerge(lists[start], lists[end], 1);
    }else{
        mid = (start+end)/2;
        return binaryMerge( mergeKListsDC(lists, start, mid), mergeKListsDC(lists, mid+1, end), 1 );
    }
}
...
struct ListNode* mergeKLists(struct ListNode** lists, int listsSize) {
    ...
    int mid = 0;
    if(listsSize==2){
        return binaryMerge(lists[0], lists[1], 1);
    }else{
        mid = listsSize/2;
        return binaryMerge( mergeKListsDC(lists, 0, mid), mergeKListsDC(lists, mid+1, listsSize-1), 1 );
    }
}

//两两归并
struct ListNode* mergeKListsWithBinaryMerge(struct ListNode** lists, int listsSize) {
    if(listsSize<2){
        return lists[0];
    }
    int i = 0;
    ...
    int completed = 0;
    int minIndex = 0;
    struct ListNode maxNode = {INT_MAX, NULL};
    ...
    if(isASC==-1){
        while(completed<listsSize){
            for(i=0; i<listsSize; i++){
                if(lists[i]!=NULL
                   && (lists[minIndex]==NULL || lists[i]->val<lists[minIndex]->val) ){
                    minIndex = i;
                }
            }
            if(lists[minIndex]==NULL
               || lists[minIndex]==&maxNode){
                return result.next;
            }
    ...
    for(i=0; i<listsSize; i++){
        tmp = binaryMerge(tmp, lists[i], isASC);
    }
    return tmp;
}
...
struct ListNode* mergeKLists0(struct ListNode** lists, int listsSize) {
    ...
    int completed = 0;
    struct ListNode maxNode = {INT_MAX, NULL};
    struct ListNode minNode = {INT_MIN, NULL};
    if(isASC==1){//升序
        while(completed<listsSize-1){
            for(i=0; i<listsSize; i++){
                if(lists[i]!=NULL
                   && (lists[minIndex]==NULL || lists[i]->val<lists[minIndex]->val) ){
                    minIndex = i;
        ...
        for(i=0; i<listsSize; i++){
            if(lists[i]!=NULL  && lists[i]!=&maxNode && lists[i]!=&minNode){
                tail->next = lists[i];
            }
        }
        return result.next;
        ...
        while(completed<listsSize-1){
            for(i=0; i<listsSize; i++){
                if(lists[i]!=NULL
                   && (lists[minIndex]==NULL || lists[i]->val>lists[maxIndex]->val ) ){
                    maxIndex = i;
                }
            }
...
//    tail->val = nums2[i];
    ...
    struct ListNode* result = mergeKLists(lists, 7);
    //mergeKLists(lists, 4);
